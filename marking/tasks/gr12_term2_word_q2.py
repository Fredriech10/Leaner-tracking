"""Marks Grade 12 Term 2 practical Question 2 from one uploaded DOCX."""
...
from marking.merkwerk.gr12_term2_q2 import Q2Document, evaluate_q2_check
...
def _checks() -> list[dict]:
    ...
    question = next(item for item in data["questions"] if item["sheet"] == "Q2")
...
def mark_for_learner(filepath: str, learner_name: str) -> dict:
    ...
    if _detect_content_type(filepath) != "word":
        ...
        return {"task_name": "CAT Grade 12 Term 2: Word Question 2", "score": 0, "total": sum(c["mark"] for c in checks), "percentage": 0, "results": results, "error": None}

    document = Q2Document(Path(filepath), learner_name)
    results, score, total = [], 0, 0
    for check in checks:
        evaluated = evaluate_q2_check(document, check)
        marks = check["mark"]
    ...
    return {"task_name": "CAT Grade 12 Term 2: Word Question 2", "score": score, "total": total, "percentage": round(score / total * 100) if total else 0, "results": results, "error": None}


def mark(filepath: str) -> dict:
    return mark_for_learner(filepath, "Learner upload")
