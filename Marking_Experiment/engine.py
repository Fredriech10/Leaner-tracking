...
class MarkingEngine:
    ...
    def run_task(self, task_definition: Dict[str, Any], file_path: Path) -> MarkingSession:
        ...
        try:
            ...
            fallback_checker = self._find_checker(program)
            if fallback_checker is None:
                raise

            def rule_checker(path: Path, rule: Dict[str, Any]) -> CheckerResult:
                return fallback_checker.check(
                    str(rule.get("domain", "")),
                    str(rule.get("type", rule.get("property", ""))),
                    rule.get("target", {}) or {},
                    rule.get("expected"),
                    path,
                )
