...
class MarksheetParser:
    def __init__(self, model: Optional[str] = None, backend: str = "ollama") -> None:
        self.backend = backend.lower()
        if self.backend != "ollama":
            raise ValueError("Only local Ollama backend is supported.")
        self.model = model or "qwen2.5-coder:7b-instruct-q4_K_M"

    def parse(self, question_paper_text: str, marksheet_text: str, program: str = "word", filename: Optional[str] = None, progress_cb: Optional[Callable[[str], None]] = None) -> ParseResult:
        ...
        try:
            ...
            response_text = self._call_ollama(prompt, progress_cb=progress_cb)
            questions = self._atomize_questions(self._extract_json(response_text))
            ...
            if questions:
                questions = self._atomize_questions(questions)
                validation_warnings = self._validate_questions(questions)
    ...
    def _parse_memo_tables(self, marksheet_text: str, program: str) -> List[Dict[str, Any]]:
        ...
        for rows in table_blocks:
            ...
            if len(header) >= 2 and any("no." in cell.lower() for cell in header) and any("criteria" in cell.lower() for cell in header):
                ...
                if parsed:
                    parsed_questions.extend(parsed)
    ...
    def _parse_criteria_table(self, rows: List[List[str]]) -> List[Dict[str, Any]]:
        ...
        for row in rows:
            ...
            marks_text = self._row_mark_text(row)
            is_numbered_criterion = bool(re.match(r"^\d+(?:\.\d+)*[a-z]?$", first.strip(), re.IGNORECASE))

            # Section header: non-empty first col without a criterion number.
            if first and not is_numbered_criterion:
                # flush any pending partial check
                if pending:
                    questions.append(pending)
                    pending = None
                section_number = first if first != second else None
                section_title = second if first != second else first
                section_index = 1
                continue

            # Check row: standard memo tables either use:
            #   empty first col + criteria in second col (sectioned memos), or
            #   question number in first col + criteria in second col (official MGs).
            if (not first and second) or (is_numbered_criterion and second):
                ...
                if is_numbered_criterion and not marks_text.strip() and len(second.split()) <= 4:
                    if pending:
                        questions.append(pending)
                        pending = None
                ...
                if second.strip().lower() in SECTION_LABEL_KEYWORDS and not marks_text.strip():
                    if pending:
                        questions.append(pending)
                        pending = None
                    continue

                marks = self._parse_mark_value(marks_text)
                question_number = first if is_numbered_criterion else (
                    f"{section_number}.{section_index}" if section_number else str(len(questions) + 1)
                )

                # Detect measurement-only follow-up rows (e.g. "Measurement set to 0.5 cm")
                # and merge them into the previous pending check.
                # BUT: do not add marks together. Treat each row as its own atomic check.
                build_description = second
                if not is_numbered_criterion and section_title:
                    build_description = f"{section_title} - {second}"

                if pending and not is_numbered_criterion and self._is_measurement_row(second):
                    questions.append(pending)
                    measurement_question = dict(pending)
                    self._merge_measurement(measurement_question, build_description)

                    measurement_question["question_number"] = f"{section_number}.{section_index}"
                    measurement_question["marks"] = 1

                    questions.append(measurement_question)
                    pending = None
                    section_index += 1
                    continue
                ...
                if is_numbered_criterion:
                    split_descriptions = self._split_criteria_descriptions(second)
                    if len(split_descriptions) > 1:
                        for split_idx, split_description in enumerate(split_descriptions, start=1):
                            split_question = self._build_question_from_description(
                                f"{question_number}.{split_idx}",
                                split_description,
                                1,
                            )
                            if split_question:
                                questions.append(split_question)
                        section_index += 1
                        continue

                question = self._build_question_from_description(question_number, build_description, marks)
                if question:
                    # If this looks like it will have a follow-up measurement row, hold it
                    if not is_numbered_criterion and self._expects_measurement_followup(second):
                        pending = question
                    else:
    ...
    def _row_mark_text(self, row: List[str]) -> str:
    ...
    def _split_criteria_descriptions(self, description: str) -> List[str]:
        ...
        if len(heading.split()) <= 5 and checks:
            return [f"{heading} - {check}" for check in checks]

        return parts

    def _atomize_questions(self, questions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Ensure every generated check is independently markable and worth one mark."""
            ...
            if isinstance(rules, list):
                for idx, rule in enumerate(rules, start=1):
                    if not isinstance(rule, dict):
                        continue
                    merged = {
                        "question_number": f"{question.get('question_number', '')}.{idx}",
                        "description": rule.get("description") or question.get("description", ""),
                        "domain": rule.get("domain", question.get("domain", "")),
                        "type": rule.get("type", question.get("type", "")),
                        "target": rule.get("target", question.get("target", {})),
                        "expected": rule.get("expected", question.get("expected")),
                        "marks": 1,
                    }
                    atomic.extend(self._split_expected_fields(merged))
                continue

            q = dict(question)
            q["marks"] = 1
            atomic.extend(self._split_expected_fields(q))
        return atomic

    def _split_expected_fields(self, question: Dict[str, Any]) -> List[Dict[str, Any]]:
        expected = question.get("expected")
        if question.get("type") in {"line_spacing", "border", "page_border", "drop_cap"}:
            return [question]
        if not isinstance(expected, dict) or len(expected) <= 1:
            return [question]

        atomic: List[Dict[str, Any]] = []
        for idx, (key, value) in enumerate(expected.items()):
            suffix = chr(ord("a") + idx) if idx < 26 else f"_{idx + 1}"
            sub_question = dict(question)
            sub_question["question_number"] = f"{question.get('question_number', '')}{suffix}"
            sub_question["description"] = f"{question.get('description', '').strip()} - {key}".strip(" -")
            sub_question["expected"] = {key: value}
            sub_question["marks"] = 1
            atomic.append(sub_question)
        return atomic
    ...
    def _build_question_from_description(self, question_number: str, description: str, marks: int) -> Optional[Dict[str, Any]]:
        ...
        if "paper size" in desc_lower or "document size" in desc_lower or "page size" in desc_lower:
            ...
            if values:
                ...
                expected = {"all": True}
        elif "border applied to document" in desc_lower:
            check_type = "page_border"
            style_match = re.search(r"double|single|thick|dashed|dotted", desc_lower)
            expected = {"style": style_match.group(0)} if style_match else True
            ...
            expected = True
        elif "style" in desc_lower and ("double-line" in desc_lower or "double line" in desc_lower) and ("colour" in desc_lower or "color" in desc_lower):
            check_type = "page_border"
            color_match = re.search(r"(blue|red|green|yellow|black|white|gray|grey)", desc_lower)
            expected = {"style": "double"}
            if color_match:
                expected["color"] = color_match.group(1)
        elif (
            ("colour set to" in desc_lower or "color set to" in desc_lower or ("colour" in desc_lower and "watermark" not in desc_lower and "border" not in desc_lower))
            and "paragraph border" not in desc_lower
        ):
            check_type = "watermark"
            color_match = re.search(r"(blue|red|green|yellow|black|white|gray|grey)", desc_lower)
        ...
        # --- paragraph_formatting checks ---
            ...
            if "justify" in desc_lower or "justified" in desc_lower:
                ...
                expected = "left"
        elif "paragraph spacing" in desc_lower and "spacing" in desc_lower and "pt" in desc_lower:
            domain = "paragraph_formatting"
        ...
        elif "from the left" in desc_lower or "left indentation" in desc_lower or "left indent" in desc_lower:
            domain = "paragraph_formatting"
            ...
            expected = float(val_match.group(1)) if val_match else True
            ...
            expected = {}
            if style_match:
                expected["style"] = style_match.group(1)
            if color_match:
                expected["color"] = color_match.group(1)
            if width_match:
                expected["width_pt"] = float(width_match.group(1))
            if not expected:
                expected = True
        elif ("colour set to" in desc_lower or "color set to" in desc_lower or "width to" in desc_lower) and "pt" in desc_lower:
            domain = "paragraph_formatting"
            check_type = "border"
            target = {"locator": "document"}
            width_match = re.search(r"(\d+(?:\.\d+)?)\s*pt", desc_lower)
        ...
        elif "position" in desc_lower and "dropped" in desc_lower:
            domain = "paragraph_formatting"
            check_type = "drop_cap"
            target = {"locator": "document"}
            expected = {"position": "dropped"}
        elif "lines to drop" in desc_lower:
            domain = "paragraph_formatting"
            check_type = "drop_cap"
            target = {"locator": "document"}
            lines_match = re.search(r"(\d+)", desc_lower)
            expected = {"lines": int(lines_match.group(1)) if lines_match else 3}
    ...
    def _call_ollama(self, prompt: str, progress_cb: Optional[Callable[[str], None]] = None) -> str:
        """Call local Ollama; try HTTP streaming first, then fall back to CLI.

        progress_cb: optional callback called with incremental text as it arrives.
        Returns final output string.
        """
        # Try HTTP streaming endpoint first
        try:
            return self._call_ollama_http_stream(prompt, progress_cb=progress_cb)
        except Exception:
            # fall back to CLI run
    ...
    def _call_ollama_cli(self, prompt: str) -> str:
        ...
        try:
            result = subprocess.run(
                ...
                env=env,
                input=prompt,
                timeout=int(os.environ.get("MARKING_LLM_TIMEOUT", str(DEFAULT_LLM_TIMEOUT_SECONDS))),
            )
    ...
    def _call_ollama_http_stream(self, prompt: str, progress_cb: Optional[Callable[[str], None]] = None) -> str:
        """Call the local Ollama HTTP API /api/generate with streaming and return accumulated output.

        This uses a simple chunked read loop so callers can receive incremental text via progress_cb.
        """
        import http.client
        import json
        from .config import DEFAULT_OLLAMA_HOST, DEFAULT_LLM_TIMEOUT_SECONDS

        host = DEFAULT_OLLAMA_HOST.split(":")[0]
        port = int(DEFAULT_OLLAMA_HOST.split(":")[1]) if ":" in DEFAULT_OLLAMA_HOST else 11434

        payload = json.dumps({"model": self.model, "prompt": prompt, "stream": True})

        conn = http.client.HTTPConnection(host, port, timeout=10)
        try:
            conn.request("POST", "/api/generate", body=payload, headers={"Content-Type": "application/json", "Accept": "text/event-stream, application/json, */*"})
            resp = conn.getresponse()
        except Exception as exc:
            raise RuntimeError(f"Ollama HTTP request failed: {exc}")

        if resp.status >= 400:
            body = resp.read().decode("utf-8", errors="replace")
            raise RuntimeError(f"Ollama HTTP API error {resp.status}: {body}")

        out_parts: List[str] = []
        timeout_seconds = int(os.environ.get("MARKING_LLM_TIMEOUT", str(DEFAULT_LLM_TIMEOUT_SECONDS)))
        start = time.time()
        try:
            # Read in small chunks; resp.read will block until data is available or connection closes
            while True:
                ...
                if progress_cb:
                    progress_cb("".join(out_parts))
                if (time.time() - start) > timeout_seconds:
                    raise RuntimeError(f"Ollama HTTP stream timed out after {timeout_seconds}s")
        finally:
            try:
                conn.close()
            except Exception:
        ...
        if not output:
            raise RuntimeError("Ollama HTTP generate returned no output.")
        return output
    ...
    def _build_prompt(
        ...
        return (
            ...
            "TASK: Extract every markable check from the question paper and memo into structured marking JSON.\n"
            "The required output is a computer-readable marking map like structured_expectations.json.\n"
            "If a memo row is worth more than 1 mark, split it into separate atomic checks.\n\n"
            ...
            "7. Every check must be binary: pass earns 1, fail earns 0.\n"
            "8. Never output marks greater than 1.\n"
            "9. Split compound instructions into separate checks. Example: 'make heading bold and red' becomes one bold check and one color check.\n"
            "10. Split compound expected objects. Example: margins top/bottom/left/right become separate one-mark checks.\n"
            "11. Use stable targets: text snippets, heading names, table row/column/cell, paragraph index after heading, sheet/cell/range, or document-level locator.\n"
            "12. For table-based memos, each markable criterion after headers is a check; extract criteria into domain/type/target/expected.\n"
            "13. Map common checks: alignment to paragraph_formatting/alignment, font size to font/size, etc.\n"
            "14. Preserve the memo question number, adding letter suffixes for split checks: 1.2a, 1.2b, 1.2c.\n\n"
            "EXAMPLE OUTPUT:\n"
            "{\n"
            "  \"questions\": [\n"
            "    {\n"
    ...
    def _extract_json(self, response_text: str) -> List[Dict[str, Any]]:
        response_text = self._clean_llm_json_text(response_text)
        for candidate in self._extract_json_candidates(response_text):
            for cleaned_candidate in (candidate, self._escape_string_newlines(candidate)):
                try:
    ...
    def _clean_llm_json_text(self, text: str) -> str:
        # Ollama/terminal streams can include ANSI escape sequences such as
        # ESC[K and cursor movement fragments; those break json.loads().
        text = re.sub(r"\x1b\[[0-9;?]*[ -/]*[@-~]", "", text)
        text = re.sub(r"[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]", "", text)
        return text

    def _escape_string_newlines(self, text: str) -> str:
        out = []
        in_string = False
        escaped = False
        for ch in text:
